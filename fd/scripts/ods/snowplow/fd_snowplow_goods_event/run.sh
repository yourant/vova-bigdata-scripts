...
shell_path="/mnt/vova-bigdata-scripts/fd/ods/snowplow/fd_snowplow_goods_event"

#筛选对应数据到对应表中
hive -hiveconf "dt_filter=$dt_filter" -f ${shell_path}/ods_fd_snowplow_goods_event.hql
if [ $? -ne 0 ]; then
  exit 1
fi
echo "ods_fd_snowplow_goods_event table is finished !"
