...
#将全量数据放到arc表中
hive -hiveconf dt=$dt -hiveconf dt_format=$dt_format -f ${shell_path}/${table_name}/${table_name}_arc.hql
#如果脚本失败，则报错
if [ $? -ne 0 ];then
  exit 1
fi
...
#snapshot表
hive -hiveconf dt=$dt -hiveconf dt_format=$dt_format -f ${shell_path}/${table_name}/${table_name}_snapshot.hql
#如果脚本失败，则报错
if [ $? -ne 0 ];then
  exit 1
fi
