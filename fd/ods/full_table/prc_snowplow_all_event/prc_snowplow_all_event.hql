CREATE table if not exists ods.ods_fd_prc_snowplow_all_event
(
    app_id               STRING,
    platform             STRING,
    project              STRING,
    ...
    oods_event_struct   array<struct<list_uri : String, list_type : String, virtual_goods_id : String, picture
...
SELECT common_struct.app_id,
       ...
       '' as referrer_page_code,
       common_struct.page_code,
       common_struct.user_unique_id,
       case
            when common_struct.platform = 'web' and common_struct.session_idx = 1 then 'new'
            when common_struct.platform = 'web' and common_struct.session_idx > 1 then 'old'
            when common_struct.platform = 'mob' and common_struct.session_idx = 1 then 'new'
            when common_struct.platform = 'mob' and common_struct.session_idx > 1 then 'old'
            end                                                          as is_new_user,
        case
            when common_struct.user_id = '0' then 'no'
            when common_struct.user_id is null then 'no'
            when common_struct.user_id = 'undefined' then 'no'
            else "yes"
            end                                                          as is_login_in,
       regexp_replace(common_struct.abtest, '&amp;', '&') as abtest,
       ...
       nvl(regexp_extract(common_struct.`page_url`, "(/r|-r|landing/)([0-9]+)", 2), '') as `url_route_sn`,
       nvl(regexp_extract(common_struct.`page_url`, "(m|/|mid=)([0-9]{5,10})", 2), '')  as `url_virtual_goods_id`,
       goods_event_struct,
...
and date(from_utc_timestamp(from_unixtime(unix_timestamp(common_struct.derived_ts,"yyyy-MM-dd'T'hh:mm:ss.SSS'Z'"),"yyyy-MM-dd hh:mm:ss"),'PRC')) = '${hiveconf:dt}';
