...
#脚本路径
sql="
alter table ods_fd_vb.ods_fd_order_status_change_history_arc drop if not exists partition (pt='$pt');
...
new_value,
create_time
from ods_fd_vb.ods_fd_order_status_change_history_arc where pt='$pt_last'
UNION ALL
select
pt,
id,
...
new_value,
create_time
from ods_fd_vb.ods_fd_order_status_change_history_inc where pt = '$pt'
)arc
) tab where tab.rank = 1;
"

spark-sql --conf "spark.sql.parquet.writeLegacyFormat=true"  --conf "spark.app.name=fd_order_status_change_history_arc_gaohaitao"   --conf "spark.sql.output.coalesceNum=40" --conf "spark.dynamicAllocation.minExecutors=40" --conf "spark.dynamicAllocation.initialExecutors=60" -e "$sql"
