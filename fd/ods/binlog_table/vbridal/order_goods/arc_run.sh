...
INSERT OVERWRITE table ods_fd_vb.ods_fd_order_goods_arc PARTITION (pt='$pt')
...
display_heel_type_price_exchange
from ods_fd_vb.ods_fd_order_goods_arc where pt = '$pt_last'
union all
select
pt,
rec_id,
...
spark-sql --conf "spark.sql.parquet.writeLegacyFormat=true"  --conf "spark.app.name=fd_order_goods_arc_gaohaitao"   --conf "spark.sql.output.coalesceNum=40" --conf "spark.dynamicAllocation.minExecutors=40" --conf "spark.dynamicAllocation.initialExecutors=60" -e "$sql"
