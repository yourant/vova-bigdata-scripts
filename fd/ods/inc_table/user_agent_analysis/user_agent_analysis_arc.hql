...
select user_agent_id, is_app, device_type, os_type, version, device_id, uuid, project_name, device_name, browser, idfa, idfv, imei, android_id, ga_id
from (
    select user_agent_id, is_app, device_type, os_type, version, device_id, uuid, project_name, device_name, browser, idfa, idfv, imei, android_id, ga_id, 
        row_number () OVER (PARTITION BY user_agent_id ORDER BY dt DESC) AS rank
    from(
        select 
            dt,
            user_agent_id,
        ...
        UNION
         ...
         select 
            ...
            android_id,
            ga_id
        from ods_fd_vb.ods_fd_user_agent_analysis_inc
