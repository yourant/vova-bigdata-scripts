...
sql="
drop table if exists data_report.ads_fd_goods_display_order_artemis_new;
drop table if exists data_report.ads_fd_goods_display_order_artemis_${pt};

CREATE TABLE IF NOT EXISTS \`data_report\`.\`ads_fd_goods_display_order_artemis_new\` (
...
CREATE TABLE IF NOT EXISTS \`data_report\`.\`ads_fd_goods_display_order_artemis\` (
  \`id\` int(11) NOT NULL AUTO_INCREMENT,
  \`goods_id\` int(11) COMMENT '商品ID',
  \`project_name\` varchar(50) COMMENT '组织',
  \`platform\` varchar(50) COMMENT '平台',
...
--m 1 \
--table ads_fd_goods_display_order_artemis_new \
...
rename table data_report.ads_fd_goods_display_order_artemis to data_report.ads_fd_goods_display_order_artemis_${pt},data_report.ads_fd_goods_display_order_artemis_new to data_report.ads_fd_goods_display_order_artemis;
EOF
echo "-------rename结束--------"
