...
sql="
CREATE TABLE IF NOT EXISTS \`vbridal\`.\`goods_display_order_artemis_interval_new\` (
...
CREATE TABLE IF NOT EXISTS \`vbridal\`.\`goods_display_order_artemis_interval\` (
  \`id\` int(11) NOT NULL AUTO_INCREMENT,
  \`goods_id\` int(11) COMMENT '商品ID',
  \`project_name\` varchar(50) COMMENT '组织',
  \`platform\` varchar(50) COMMENT '平台',
...
--m 1 \
--table goods_display_order_artemis_interval_new \
...
rename table vbridal.goods_display_order_artemis_interval to vbridal.goods_display_order_artemis_interval_${pt},vbridal.goods_display_order_artemis_interval_new to vbridal.goods_display_order_artemis_interval;
EOF
echo "-------rename结束--------"
