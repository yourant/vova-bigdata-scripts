...
union all
select
'商品销售总金额',round(tmp1.day_gmv,2),'',round(tmp1.mon_gmv,2),'',4
from tmp.tmp_ad_income_01 tmp1
union all
select
'退款金额',round(tmp1.day_gmv * ${refund_order_cnt_rate},2),concat(round(${refund_order_cnt_rate} / (1- ${refund_order_cnt_rate}) * 100,2),'%'),round(tmp1.mon_gmv * ${refund_order_cnt_rate},2),concat(round(${refund_order_cnt_rate} / (1- ${refund_order_cnt_rate}) * 100,2),'%'),5
from tmp.tmp_ad_income_01 tmp1
union all
select
'商品销售净收入',round(tmp1.day_gmv * (1 - ${refund_order_cnt_rate}),2),'100%',round(tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}),2),'100%',6
from tmp.tmp_ad_income_01 tmp1
union all
select
'红包消耗总金额',
round(tmp1.day_bonus * (1 - ${refund_amount_rate}),2),
concat(round(abs(tmp1.day_bonus * (1 - ${refund_amount_rate}) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.mon_bonus * (1 - ${refund_amount_rate}),2),
concat(round(abs(tmp1.mon_bonus * (1 - ${refund_amount_rate}) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),7
from tmp.tmp_ad_income_01 tmp1
union all
select
'发货红包金额',
round(tmp1.day_bonus,2),
concat(round(abs(tmp1.day_bonus / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.mon_bonus,2),
concat(round(abs(tmp1.mon_bonus / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),8
from tmp.tmp_ad_income_01 tmp1
union all
select
'退货红包金额',
round(tmp1.day_bonus * -${refund_amount_rate},2),
concat(round(abs(tmp1.day_bonus * ${refund_amount_rate} / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.mon_bonus * -${refund_amount_rate},2),
concat(round(abs(tmp1.mon_bonus * ${refund_amount_rate} / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),9
from tmp.tmp_ad_income_01 tmp1
union all
select
'物流费盈亏',
round(tmp1.day_shou_express - ${pay_express_rate} * tmp1.day_gmv,2),
concat(round(abs((tmp1.day_shou_express - ${pay_express_rate} * tmp1.day_gmv) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.mon_shou_express - ${pay_express_rate} * tmp1.mon_gmv,2),
concat(round(abs((tmp1.mon_shou_express - ${pay_express_rate} * tmp1.mon_gmv) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),10
from tmp.tmp_ad_income_01 tmp1
union all
select
'代收快递费',
round(tmp1.day_shou_express,2),
concat(round(abs(tmp1.day_shou_express / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.mon_shou_express,2),
concat(round(abs(tmp1.mon_shou_express / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),11
from tmp.tmp_ad_income_01 tmp1
union all
select
'代付快递费',
round(${pay_express_rate} * tmp1.day_gmv,2),
concat(round(abs(${pay_express_rate} * tmp1.day_gmv / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(${pay_express_rate} * tmp1.mon_gmv,2),
concat(round(abs(${pay_express_rate} * tmp1.mon_gmv / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),12
from tmp.tmp_ad_income_01 tmp1
union all
select
'商品采购成本',
round(tmp1.union_cost_day,2),
concat(round(abs(tmp1.union_cost_day / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.unit_cost_month,2),
concat(round(abs(tmp1.unit_cost_month / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),13
from tmp.tmp_ad_income_01 tmp1
union all
select
'总成本',
round(tmp1.union_cost_day + tmp4.ad_cost_day + tmp1.order_amount_day * ${pay_free_rate_1} + tmp1.day_order_cnt * ${pay_free_rate_2},2),
concat(round(abs((tmp1.union_cost_day + tmp4.ad_cost_day + tmp1.order_amount_day * ${pay_free_rate_1} + tmp1.day_order_cnt * ${pay_free_rate_2}) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.unit_cost_month + tmp4.ad_cost_month + tmp1.order_amount_month * ${pay_free_rate_1} + tmp1.mon_order_cnt * ${pay_free_rate_2},2),
concat(round(abs((tmp1.unit_cost_month + tmp4.ad_cost_month + tmp1.order_amount_month * ${pay_free_rate_1} + tmp1.mon_order_cnt * ${pay_free_rate_2}) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),14
from tmp.tmp_ad_income_01 tmp1 join tmp.tmp_ad_income_02 tmp4 on 1 = 1
union all
select
'广告费',
round(tmp4.ad_cost_day,2),
concat(round(abs(tmp4.ad_cost_day / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp4.ad_cost_month,2),
concat(round(abs(tmp4.ad_cost_month / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),15
...
concat(round(abs((tmp1.order_amount_day * ${pay_free_rate_1} + tmp1.day_order_cnt * ${pay_free_rate_2}) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.order_amount_month * ${pay_free_rate_1} + tmp1.mon_order_cnt * ${pay_free_rate_2},2),
concat(round(abs((tmp1.order_amount_month * ${pay_free_rate_1} + tmp1.mon_order_cnt * ${pay_free_rate_2}) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),16
from tmp.tmp_ad_income_01 tmp1
union all
select
'毛利',
...
select
'毛利率%',
concat(round(abs((tmp1.day_gmv * (1 - ${refund_order_cnt_rate}) + tmp1.day_bonus * (1 - ${refund_amount_rate}) + tmp1.day_shou_express - ${pay_express_rate} * tmp1.day_gmv - (tmp1.union_cost_day + tmp4.ad_cost_day + tmp1.order_amount_day * ${pay_free_rate_1} + tmp1.day_order_cnt * ${pay_free_rate_2})) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
'',
concat(round(abs((tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}) + tmp1.mon_bonus * (1 - ${refund_amount_rate}) + tmp1.mon_shou_express - ${pay_express_rate} * tmp1.mon_gmv - (tmp1.unit_cost_month + tmp4.ad_cost_month + tmp1.mon_gmv * ${pay_free_rate_1} + tmp1.mon_order_cnt * ${pay_free_rate_2})) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
'',18
from tmp.tmp_ad_income_01 tmp1
join tmp.tmp_ad_income_02 tmp4 on 1 = 1
union all
...
concat(round(abs((tmp1.day_gmv * ${employee_money_order} + tmp1.day_gmv * ${stock_order} + ${house_amount} + ${computer_amount}) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
...
concat(round(abs((tmp1.mon_gmv * ${employee_money_order} + tmp1.mon_gmv * ${stock_order} + day('${cur_date}') * ${house_amount} + day('${cur_date}') * ${computer_amount}) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),19
from tmp.tmp_ad_income_01 tmp1
union all
select
'人员工资',
round(tmp1.day_gmv * ${employee_money_order},2),
concat(round(abs(tmp1.day_gmv * ${employee_money_order} / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.mon_gmv * ${employee_money_order},2),
concat(round(abs(tmp1.mon_gmv * ${employee_money_order} / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),20
from tmp.tmp_ad_income_01 tmp1
union all
select
'仓储费',
round(tmp1.day_gmv * ${stock_order},2),
concat(round(abs(tmp1.day_gmv * ${stock_order} / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.mon_gmv * ${stock_order},2),
concat(round(abs(tmp1.mon_gmv * ${stock_order} / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),21
from tmp.tmp_ad_income_01 tmp1
union all
select
'房租',
${house_amount},
concat(round(abs(${house_amount} / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
day('${cur_date}') * ${house_amount},
concat(round(abs(day('${cur_date}') * ${house_amount} / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),22
from tmp.tmp_ad_income_01 tmp1
union all
select
'服务器费用',
${computer_amount},
concat(round(abs(${computer_amount} / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
day('${cur_date}') * ${computer_amount},
concat(abs(round(day('${cur_date}') * ${computer_amount} / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate})) * 100,2)),'%'),23
from tmp.tmp_ad_income_01 tmp1
union all
select
'净利润',
...
concat(round(abs((tmp1.day_gmv * (1 - ${refund_order_cnt_rate}) + tmp1.day_bonus * (1 - ${refund_amount_rate}) + tmp1.day_shou_express - ${pay_express_rate} * tmp1.day_gmv - (tmp1.union_cost_day + tmp4.ad_cost_day + tmp1.order_amount_day * ${pay_free_rate_1} + tmp1.day_order_cnt * ${pay_free_rate_2}) -
(tmp1.day_gmv * ${employee_money_order} + tmp1.day_gmv * ${stock_order} + ${house_amount} + ${computer_amount})) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
...
concat(abs(round((tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}) + tmp1.mon_bonus * (1 - ${refund_amount_rate}) + tmp1.mon_shou_express - ${pay_express_rate} * tmp1.mon_gmv - (tmp1.unit_cost_month + tmp4.ad_cost_month + tmp1.mon_gmv * ${pay_free_rate_1} + tmp1.mon_order_cnt * ${pay_free_rate_2}) -
(tmp1.mon_gmv * ${employee_money_order} + tmp1.mon_gmv * ${stock_order} + day('${cur_date}') * ${house_amount} + day('${cur_date}') * ${computer_amount})) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate})) * 100,2)),'%'),24
from tmp.tmp_ad_income_01 tmp1
join tmp.tmp_ad_income_02 tmp4 on 1 = 1
;
...
insert overwrite table tmp.tmp_ad_income_03
    select count(distinct if(to_date(a.order_time) = '${cur_date}', a.order_id,
         ...
         sum(if(to_date(a.order_time) = '${cur_date}', a.order_amount, 0))   order_amount_day,          --下单维度总收款金额 天
         sum(if(from_unixtime(unix_timestamp(a.order_time), 'yyyy-MM') = from_unixtime(unix_timestamp(to_date('${cur_date}')), 'yyyy-MM')  and to_date(a.order_time) <= '${cur_date}' ,a.order_amount,0))    order_amount_month,  --下单维度总收款金额 月
         sum(if(to_date(a.order_time) = '${cur_date}', i.should_amount, 0))   should_express_amount_day,          --代付快递费 天
         sum(if(from_unixtime(unix_timestamp(a.order_time), 'yyyy-MM') = from_unixtime(unix_timestamp(to_date('${cur_date}')), 'yyyy-MM')  and to_date(a.order_time) <= '${cur_date}' ,i.should_amount,0))    should_express_amount_month,  --代付快递费 月
...
group by order_id) h on a.order_id = h.order_id
left join (select order_id,sum(should_amount) should_amount from (
select nvl(t.should_amount / usd_currency_conversion_rate, 0.00) as should_amount,
t.order_id
from (
select os.order_id,
      fflo.should_amount,
      nvl(ucc.currency_conversion_rate, 1.0)                                                    as usd_currency_conversion_rate,
      row_number()
              OVER (PARTITION BY os.order_id ORDER BY ucc.currency_conversion_shanghai_ts DESC) AS currency_rn
from ods_fd_ecshop.ods_fd_fly_fish_logistics_order fflo
        join ods_fd_romeo.ods_fd_order_shipment os on os.shipment_id = fflo.shipment_id
        left join (SELECT currency_conversion_rate
                        , to_currency_code
                        , currency_conversion_date                                    as currency_conversion_shanghai_ts
                        , to_utc_timestamp(currency_conversion_date, 'Asia/Shanghai') as currency_conversion_utc_ts
...
where currency_rn = 1
) t2 group by order_id) i on a.order_id = i.order_id
...
concat(round(abs((tmp1.day_shou_express - tmp1.should_express_amount_day) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.mon_shou_express -tmp1.should_express_amount_month,2),
concat(round(abs((tmp1.mon_shou_express -tmp1.should_express_amount_month) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),10
from tmp.tmp_ad_income_03 tmp1
union all
select
'代收快递费',
...
select
'代付快递费',
round(tmp1.should_express_amount_day,2),
concat(round(abs(tmp1.should_express_amount_day / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.should_express_amount_month,2),
concat(round(abs(tmp1.should_express_amount_month / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),12
from tmp.tmp_ad_income_03 tmp1
union all
select
'商品采购成本',
...
select
'总成本',
round(tmp1.union_cost_day + (tmp1.ad_cost_day) + tmp1.order_amount_day * ${pay_free_rate_1} + tmp1.day_order_cnt * ${pay_free_rate_2},2),
concat(round(abs((tmp1.union_cost_day + (tmp1.ad_cost_day) + tmp1.order_amount_day * ${pay_free_rate_1} + tmp1.day_order_cnt * ${pay_free_rate_2}) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.unit_cost_month + (tmp1.ad_cost_month) + tmp1.order_amount_month * ${pay_free_rate_1} + tmp1.mon_order_cnt * ${pay_free_rate_2},2),
concat(round(abs((tmp1.unit_cost_month + (tmp1.ad_cost_month) + tmp1.order_amount_month * ${pay_free_rate_1} + tmp1.mon_order_cnt * ${pay_free_rate_2}) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),14
from tmp.tmp_ad_income_03 tmp1
union all
select
'广告费',
round(tmp1.ad_cost_day,2),
concat(round(abs((tmp1.ad_cost_day) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.ad_cost_month,2),
concat(round(abs((tmp1.ad_cost_month) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),15
from tmp.tmp_ad_income_03 tmp1
union all
select
'支付手续费',
...
select
'毛利率%',
concat(round(abs((tmp1.day_gmv * (1 - ${refund_order_cnt_rate}) + tmp1.day_bonus * (1 - ${refund_amount_rate}) + tmp1.day_shou_express - tmp1.should_express_amount_day - (tmp1.union_cost_day + (tmp1.ad_cost_day) + tmp1.order_amount_day * ${pay_free_rate_1} + tmp1.day_order_cnt * ${pay_free_rate_2})) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
'',
concat(round(abs((tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}) + tmp1.mon_bonus * (1 - ${refund_amount_rate}) + tmp1.mon_shou_express -tmp1.should_express_amount_month - (tmp1.unit_cost_month + (tmp1.ad_cost_month) + tmp1.mon_gmv * ${pay_free_rate_1} + tmp1.mon_order_cnt * ${pay_free_rate_2})) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
'',18
from tmp.tmp_ad_income_03 tmp1
union all
select
...
concat(round(abs((tmp1.day_gmv * ${employee_money_send} + tmp1.day_gmv * ${stock_send} + ${house_amount} + ${computer_amount}) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.mon_gmv * ${employee_money_send} + tmp1.mon_gmv * ${stock_send} + day('${cur_date}') * ${house_amount} + day('${cur_date}') * ${computer_amount},2),
concat(round(abs((tmp1.mon_gmv * ${employee_money_send} + tmp1.mon_gmv * ${stock_send} + day('${cur_date}') * ${house_amount} + day('${cur_date}') * ${computer_amount}) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),19
from tmp.tmp_ad_income_03 tmp1
union all
select
'人员工资',
round(tmp1.day_gmv * ${employee_money_send},2),
concat(round(abs(tmp1.day_gmv * ${employee_money_send} / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.mon_gmv * ${employee_money_send},2),
concat(round(abs(tmp1.mon_gmv * ${employee_money_send} / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),20
from tmp.tmp_ad_income_03 tmp1
union all
select
'仓储费',
round(tmp1.day_gmv * ${stock_send},2),
concat(round(abs(tmp1.day_gmv * ${stock_send} / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
round(tmp1.mon_gmv * ${stock_send},2),
concat(round(abs(tmp1.mon_gmv * ${stock_send} / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),21
from tmp.tmp_ad_income_03 tmp1
union all
select
'房租',
...
concat(round(abs((tmp1.day_gmv * (1 - ${refund_order_cnt_rate}) + tmp1.day_bonus * (1 - ${refund_amount_rate}) + tmp1.day_shou_express - tmp1.should_express_amount_day - (tmp1.union_cost_day + (tmp1.ad_cost_day) + tmp1.order_amount_day * ${pay_free_rate_1} + tmp1.day_order_cnt * ${pay_free_rate_2}) -
(tmp1.day_gmv * ${employee_money_send} + tmp1.day_gmv * ${stock_send} + ${house_amount} + ${computer_amount})) / (tmp1.day_gmv * (1 - ${refund_order_cnt_rate}))) * 100,2),'%'),
...
concat(abs(round((tmp1.mon_gmv * (1 - ${refund_order_cnt_rate}) + tmp1.mon_bonus * (1 - ${refund_amount_rate}) + tmp1.mon_shou_express -tmp1.should_express_amount_month - (tmp1.unit_cost_month + (tmp1.ad_cost_month) + tmp1.mon_gmv * ${pay_free_rate_1} + tmp1.mon_order_cnt * ${pay_free_rate_2}) -
(tmp1.mon_gmv * ${employee_money_send} + tmp1.mon_gmv * ${stock_send} + day('${cur_date}') * ${house_amount} + day('${cur_date}') * ${computer_amount})) / (tmp1.mon_gmv * (1 - ${refund_order_cnt_rate})) * 100,2)),'%'),24
from tmp.tmp_ad_income_03 tmp1
"
...
--env prod \
-sql "select cat_name,day_value,day_rate,month_value,month_rate from dwb.dwb_ad_income_order where pt = '${cur_date}' order by rn"  \
...
--class com.vova.utils.EmailUtil s3://vomkt-emr-rec/jar/vova-bd/dataprocess/new/vova-db-dataprocess-1.0-SNAPSHOT.jar \
--env prod \
-sql "select cat_name,day_value,day_rate,month_value,month_rate from dwb.dwb_ad_income_out_stock where pt = '${cur_date}' order by rn"  \
-head "科目,${cur_date},占比（取绝对值）,本月截止昨日累计,占比（取绝对值）"  \
-receiver "juntao@vova.com.hk,cici.liu@i9i8.com,sol.ji@vova.com.hk,qi.zhong@gmail.com,qzhong@i9i8.com,ychen@i9i8.com,yfli@i9i8.com,ytang@i9i8.com,ruth.li@i9i8.com,muqie@i9i8.com,john.wang@i9i8.com" \
