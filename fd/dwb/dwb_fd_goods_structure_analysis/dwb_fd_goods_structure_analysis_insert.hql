...
FROM (
	...
	SELECT
		...
		gppa.provider_attribute_name,
		og.goods_sales_number,
	...
	--计算商品的供应商
	 	select gpp.goods_id,gpp.is_delete,gpp.price,gpp.provider_id,
	       	   ppa.provider_id,ppa.provider_attribute_id,ppa.is_used,
	           pa.provider_attribute_id,pa.provider_attribute_name
		from ods_fd_romeo.ods_fd_goods_purchase_price gpp
		left join ods_fd_ecshop.ods_fd_ecs_provider_provider_attribute ppa on gpp.provider_id = ppa.provider_id
		left join ods_fd_ecshop.ods_fd_provider_attribute  pa on ppa.provider_attribute_id = pa.provider_attribute_id
		where  gpp.is_delete = 0
		and ppa.provider_id is not null
		and pa.provider_attribute_name in('贸综','工厂')

	 )gppa on gppa.goods_id = rgp.goods_id
	...
	where gppa.provider_attribute_name is not null --只保留贸综，工厂的数据
