...
-- INSERT OVERWRITE TABLE dwb.dwb_fd_rpt_ecommerce_funnel_report PARTITION (pt = '${hiveconf:pt}')
...
--      and (date(from_unixtime(order_time,'yyyy-MM-dd hh:mm:ss')) = '${hiveconf:pt}'
...
--           where pt BETWEEN date_sub('${hiveconf:pt}', 5) AND '${hiveconf:pt}' and event_name = 'page_view'
...
--      where pt BETWEEN date_sub('${hiveconf:pt}', 10) AND date_add('${hiveconf:pt}', 1)
--      group by session_id
--
-- ) fdpsc on fdpsc.session_id = om.sp_session_id;
--
--
-- INSERT INTO TABLE dwb.dwb_fd_rpt_ecommerce_funnel_report PARTITION (pt = '${hiveconf:pt}')
-- select fms.project
--      , fms.country
--      , fms.platform_type
--      , if(fdpsc.ga_channel is null or fdpsc.ga_channel = '', 'Others', fdpsc.ga_channel)          as ga_channel
