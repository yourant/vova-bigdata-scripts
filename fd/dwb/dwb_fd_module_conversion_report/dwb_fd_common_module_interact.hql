...
INSERT OVERWRITE TABLE dwb.dwb_fd_common_module_interact PARTITION (dt = '${hiveconf:dt}')
select project,
       domain_userid,
       session_id,
       event_name,
       ...
       ''
from ods_fd_vb.ods_fd_snowplow_element_event
where dt ='${hiveconf:dt}' and event_name in ('common_impression', 'common_click')
and REGEXP_EXTRACT(element_event_struct.element_id, 'm_jump=(.*?)(&.*?)?$', 1) <> '';


INSERT INTO TABLE dwb.dwb_fd_common_module_interact PARTITION (dt = '${hiveconf:dt}')
select project,
       domain_userid,
       session_id,
       event_name,
...
from ods_fd_vb.ods_fd_snowplow_goods_event
where dt ='${hiveconf:dt}' and event_name in ('goods_impression', 'goods_click')
...
from ods_fd_vb.ods_fd_snowplow_other_event
...
from ods_fd_vb.ods_fd_snowplow_ecommerce_event
where dt ='${hiveconf:dt}'
and   event_name ='add'
