...
SELECT 
    from_utc_timestamp(1000*unix_timestamp('${hiveconf:pt}'),'America/Los_Angeles'),
    oi.project_name,
    og.goods_id,
...
where
    oi.email  NOT REGEXP "tetx.com|i9i8.com|jjshouse.com|jenjenhouse.com|163.com|qq.com"
     and from_unixtime(oi.order_time)>= date_add(from_utc_timestamp(1000*unix_timestamp('${hiveconf:pt}'),'America/Los_Angeles'),-7)
     and from_unixtime(oi.order_time) < from_utc_timestamp(1000*unix_timestamp('${hiveconf:pt}'),'America/Los_Angeles')
     and pay_status>=1
     and gp.is_on_sale=0
