...
if [ "$#" -ne 4 ]; then
  ...
  pt_begin=$(date -d "$ts -184 hours" +"%Y-%m-%d")
  pt_one=$(date -d "$ts -160 hours" +"%Y-%m-%d")
...
hive -f ${shell_path}/tmp_fd_goods_uv_interval_create.hql
spark-sql \
  --conf "spark.app.name=${tmp_table}_7" \
  --conf "spark.dynamicAllocation.maxExecutors=150" \
  -d pt_end="${pt_end}" \
  -d pt_begin="${pt_begin}" \
  -d pt_one="${pt_one}" \
  -d pt_two="${pt_two}" \
  -f ${shell_path}/tmp_fd_goods_uv_interval_insert.hql
