...
select
     ...
     , nvl(session_table.all_sessions,0) all_sessions
     , nvl(session_table.product_view_sessions,0) product_view_sessions
     , nvl(session_table.add_sessions,0) add_sessions
     , nvl(session_table.checkout_sessions,0) checkout_sessions
     , nvl(session_table.checkout_option_sessions,0) checkout_option_sessions
     , nvl(session_table.purchase_sessions,0) purchase_sessions

     , nvl(order_table.orders,0) orders
     , nvl(order_table.goods_amount,0) goods_amount
     , nvl(order_table.bonus,0) bonus
     , nvl(order_table.shipping_fee,0) shipping_fee
     , nvl(session_table.pv_session,0) pv_session
from (
...
select
     ...
     ,sum( tab.all_sessions) all_sessions
     ,sum( tab.product_view_sessions) product_view_sessions
     ,sum( tab.add_sessions) add_sessions
     ,sum( tab.checkout_sessions )checkout_sessions
     ,sum( tab.checkout_option_sessions ) checkout_option_sessions
     ,sum( tab.purchase_sessions) purchase_sessions
     ,sum( tab.orders ) orders

     ,sum( tab.goods_amount) goods_amount
     ,sum( tab.bonus) bonus
     ,sum( tab.shipping_fee) shipping_fee
     ,sum( tab.pv_session) pv_session
from
(
    select
        /*+ REPARTITION(1) */
         ...
         , case
              when country in ('AT','BE','BG','CH','CZ','DE','DK','EE','ES','FI','FR','GB','GR','HR','HU','IE','IS'
                          ,'IT','LT','LV','MC','MD','MT','NL','NO','PL','PT','RO','RU','SE','SK','SM','UA','VA','YU','BY','LI','SI') then 'Europe'
           end as country
         , is_new_user
         , ga_channel
    ...
    where
       t3.country in ('AT','BE','BG','CH','CZ','DE','DK','EE','ES','FI','FR','GB','GR','HR','HU','IE','IS'
                       ,'IT','LT','LV','MC','MD','MT','NL','NO','PL','PT','RO','RU','SE','SK','SM','UA','VA','YU','BY','LI','SI')
)tab
group by
   tab.project
 , tab.platform_type
