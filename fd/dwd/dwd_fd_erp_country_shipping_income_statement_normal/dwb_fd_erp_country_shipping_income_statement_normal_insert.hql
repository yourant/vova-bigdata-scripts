INSERT OVERWRITE TABLE dwd.dwd_fd_erp_country_shipping_income_statement_normal PARTITION (pt='${pt}')
SELECT
    order_id
    ,party_name
    ,country_code
...
/* 组织+国家 订单销售额和订单红包花费 */
    SELECT
        ...
        ,coalesce(oi.goods_amount / oi.currency_conversion_rate, 0.00) AS sales_amount
        ,coalesce(oi.bonus / oi.currency_conversion_rate * -1, 0.00) AS coupon_cost
    ...
    LEFT JOIN (
        ...
        FROM (
            SELECT
                ...
                ,party_name
                ,pt as order_pt
            FROM dwd.dwd_fd_erp_country_income_statement_normal
