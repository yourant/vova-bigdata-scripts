...
SELECT rm.event_date,
       rm.tot_gmv,
       rm.tot_dau,
       rm.tot_dau + rmwd.dau as all_dau,
       ...
       ifnull(concat(round(rm.tot_android_1b_ret / tmp1.android_dau * 100, 2), '%'),'0.00%')       AS tot_android_1d_cohort,
       ifnull(concat(round(rm.tot_android_7b_ret / tmp7.android_dau * 100, 2), '%'),'0.00%')       AS tot_android_7d_cohort,
       ifnull(concat(round(rm.tot_android_28b_ret / tmp28.android_dau * 100, 2), '%'),'0.00%')     AS tot_android_28d_cohort,
       ifnull(concat(round(rm.new_android_1b_ret / tmp1.android_activate * 100, 2), '%'),'0.00%')   AS new_android_1d_cohort,
       ifnull(concat(round(rm.new_android_7b_ret / tmp7.android_activate * 100, 2), '%'),'0.00%')   AS new_android_7d_cohort,
       ifnull(concat(round(rm.new_android_28b_ret / tmp28.android_activate * 100, 2), '%'),'0.00%') AS new_android_28d_cohort,
       ifnull(concat(round(rm.tot_ios_1b_ret / tmp1.ios_dau * 100, 2), '%'),'0.00%')               AS tot_ios_1d_cohort,
       ifnull(concat(round(rm.tot_ios_7b_ret / tmp7.ios_dau * 100, 2), '%'),'0.00%')               AS tot_ios_7d_cohort,
       ifnull(concat(round(rm.tot_ios_28b_ret / tmp28.ios_dau * 100, 2), '%'),'0.00%')             AS tot_ios_28d_cohort,
...
<head>
    <meta charset="utf-8">
    <title>Airyclub APP 大盘及渠道报表 - {datetime}</title>
    <style type="text/css">
...
<table class="table-blue">
    <caption>Airyclub APP 每日报表 - [{datetime}] </caption>
    <thead>
    <tr>
        <th></th>
    ...
    <tr>
        ...
        <th style="padding-left: 5px;padding-right: 5px">DAU(含网站)</th>
...
'''.replace("\n", "").replace("{datetime}", datetime).replace("{market_report}", market_report)
