...
#dwd.dwd_vova_fact_act_ord_gs;
sql="
INSERT overwrite TABLE dwb.dwb_vova_market_process PARTITION (pt = '${cur_date}')
    ...
    (SELECT nvl(temp.event_date, 'all')     event_date,
     ...
     GROUP BY CUBE(temp.event_date, temp.datasource, temp.region_code)
        ...
        FROM (SELECT nvl(date(install_time), 'NALL') event_date,
                     dev.device_id,
                     nvl(datasource, 'NALL')  AS     datasource,
                     nvl(region_code, 'NALL') AS     region_code
...
SELECT nvl(temp.event_date, 'all')     event_date,
        ...
        -- lucky_gmv
        LEFT JOIN (
SELECT nvl(temp.datasource, 'all')  datasource,
       nvl(temp.region_code, 'all') region_code,
       nvl(temp.event_date, 'all')  event_date,
       sum(lck_gmv) AS              tot_lucky_gmv
FROM (
         SELECT nvl(fag.datasource, 'NA')     datasource,
                nvl(fag.rgn_code, 'NALL')     region_code,
                to_date(fag.pay_time)      AS event_date,
                fag.ord_amt + fag.ship_fee AS lck_gmv
         FROM dwd.dwd_vova_fact_act_ord_gs fag
         WHERE fag.pay_time > date_sub('${cur_date}', 60)
           AND fag.pay_sts >= 1
     ) temp
GROUP BY CUBE (temp.event_date, temp.datasource, temp.region_code)
    ) tot_lucky_gmv
                  ON tot_lucky_gmv.event_date = tot_dau.event_date
                      AND tot_lucky_gmv.region_code = tot_dau.region_code
                      AND tot_lucky_gmv.datasource = tot_dau.datasource
...
--conf "spark.app.name=dwb_vova_market_process" \
