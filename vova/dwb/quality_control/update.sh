...
nvl(sum(is_refund_7w)/count(*)*100,0) as refund_rate,
...
if(fr.refund_type_id not in (1,7) and ogs.sku_pay_status = 4  and fr.exec_refund_time>=fp.pay_time and datediff(fr.exec_refund_time,fp.pay_time) <63,1,0) as is_refund_7w,
datediff(fr.exec_refund_time,fp.pay_time) as refunded_gap,
