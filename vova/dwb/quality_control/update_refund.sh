...
if(fr.refund_type_id not in (1,7)  and ogs.sku_pay_status > 1 and ((fr.rr_audit_status = 'audit_passed' and datediff(fr.rr_audit_time,fp.confirm_time)<105) or (fr.rr_audit_status != 'audit_passed' and  ogs.sku_pay_status = 4 and datediff(fr.exec_refund_time,fp.confirm_time)<105)),1,0) as is_refund_15w,
if(fr.refund_type_id not in (1,7)  and ogs.sku_pay_status > 1 and ((fr.rr_audit_status = 'audit_passed' and datediff(fr.rr_audit_time,fp.confirm_time)<105) or (fr.rr_audit_status != 'audit_passed' and ogs.sku_pay_status = 4 and datediff(fr.exec_refund_time,fp.confirm_time)<105)) and fr.refund_reason_type_id not in (8,9) ,1,0) as is_refund_15w_nlrf,
if(fr.refund_type_id not in (1,7)  and ogs.sku_pay_status > 1 and ((fr.rr_audit_status = 'audit_passed' and datediff(fr.rr_audit_time,fp.confirm_time)<105) or (fr.rr_audit_status != 'audit_passed' and ogs.sku_pay_status = 4 and datediff(fr.exec_refund_time,fp.confirm_time)<105)) and fr.refund_reason_type_id in (8,9) ,1,0) as is_refund_15w_lrf,
