...
sql="
with tmp_cmb_1 as (
select nvl(og.region_code, 'all') as region_code,
...
sum(if((og.mct_id in (26414,11630,36655) and g.brand_id = 0) or (vg.brand_level in (1,2,3,4)), 1, 0))          as my_no_brd_cnt,
sum(if((og.mct_id not in (26414,11630,36655) and g.brand_id = 0) or (vg.brand_level in (1,2,3,4)), 1, 0))         as oth_no_brd_cnt,
...
inner join dim.dim_vova_goods g using (goods_id)
inner join ods_vova_vts.ods_vova_order_goods_extra oge on oge.order_goods_id = og.order_goods_id
left join ods_vova_vts.ods_vova_order_goods_extension oget on oget.rec_id = og.order_goods_id and oget.ext_name = 'container_transportation_shipping_fee'
left join dwd.dwd_vova_fact_pay fp on og.order_goods_id = fp.order_goods_id
left join ods_vova_vts.ods_vova_goods vg on og.goods_id = vg.goods_id
where og.sku_order_status = 1
and date(og.order_time) = '${cur_date}'
and to_date(fp.pay_time) = '${cur_date}'
and og.datasource = 'vova'
group by cube (og.region_code)
)

insert overwrite table dwb.dwb_vova_monitor_cmb PARTITION (pt = '${cur_date}')
...
my_sku
from tmp_cmb_1 a
left join (select nvl(fp.region_code,'all') region_code,sum(fp.shop_price * fp.goods_number + fp.shipping_fee) gmv from dwd.dwd_vova_fact_pay fp
					...
					group by cube(fp.region_code)) b
