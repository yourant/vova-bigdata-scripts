  ...
  (
    ...
    (
      select
        ...
        page_code,
        list_type,
        device_id impression_device_id,
        'NA' click_device_id,
        0 clicks,
        1 impressions
      from
      ...
      select
        ...
        page_code,
        list_type,
        'NA' impression_device_id,
        device_id click_device_id,
        1 clicks,
        0 impressions
      from
    ...
    on t1.virtual_goods_id=g.virtual_goods_id
  ) t2
  where t2.datasource in (select DISTINCT data_domain data_domain from ods_vova_vtsf.ods_vova_acg_app)
) t3
group by datasource,region_code,first_cat_name,second_cat_name,third_cat_name,is_brand,rec_page_code with cube
-- 只为把 app-group 中的拆出来再算一遍
union all
SELECT
  datasource,
  region_code,
...
(
  ...
  (
    ...
    where t2.datasource in (
      select DISTINCT data_domain data_domain from ods_vova_vtsf.ods_vova_acg_app
      where data_domain not in ('vova', 'airyclub')
      )
...
(
  SELECT
    -- nvl(py.datasource, 'NA')      datasource,
    case when py.datasource = 'vova' then 'vova'
      when py.datasource = 'airyclub' then 'airyclub'
      else 'app-group'
      end datasource,
  ...
  on ddog.order_goods_id = py.order_goods_id
  left join
    dim.dim_vova_goods g
  on g.goods_id=py.goods_id
  left join
  (
    select *
    from
  ...
  on py.order_goods_id=oc.order_goods_id
  where to_date(py.pay_time)='${cur_date}'
    ...
    and py.datasource in (select DISTINCT data_domain data_domain from ods_vova_vtsf.ods_vova_acg_app)
) t
group by datasource,first_cat_name,second_cat_name,third_cat_name,region_code,is_brand,rec_page_code with cube
-- 无脑 union
union all
SELECT
  datasource,
...
(
  ...
  (
    ...
    where to_date(py.pay_time)='${cur_date}'
      -- and (py.from_domain like '%api.vova%' or py.from_domain like '%api.airyclub%')
      and (ddog.order_tag not like '%luckystar_activity_id%' or ddog.order_tag is null)
      and py.datasource in (
        select
          DISTINCT data_domain data_domain
        from
          ods_vova_vtsf.ods_vova_acg_app
        where data_domain not in ('vova', 'airyclub')
      )
  ) t
  group by datasource,first_cat_name,second_cat_name,third_cat_name,region_code,is_brand,rec_page_code with cube
...
(
  SELECT
    ...
    case when c.datasource = 'vova' then 'vova'
      when c.datasource = 'airyclub' then 'airyclub'
      else 'app-group'
      end datasource,
    ...
    device_id
  from
    dwd.dwd_vova_fact_cart_cause_v2 c
  left join
    dim.dim_vova_goods g
  on c.virtual_goods_id = g.virtual_goods_id
  where c.pt='$cur_date' and c.pre_page_code is not null
    and c.datasource in (select DISTINCT data_domain data_domain from ods_vova_vtsf.ods_vova_acg_app)
) t
group by datasource,first_cat_name,second_cat_name,third_cat_name,region_code,is_brand,rec_page_code with cube
...
LEFT JOIN
  tmp2 t2
ON t1.datasource = t2.datasource
...
LEFT JOIN
  tmp3 t3
ON t1.datasource = t3.datasource
...
LEFT JOIN
  tmp4 t4
ON t1.first_cat_name = t4.first_cat_name
  AND t1.second_cat_name = t4.second_cat_name
  AND t1.third_cat_name = t4.third_cat_name
  AND t1.is_brand = t4.is_brand
WHERE t1.region_code IN ('all', 'GB', 'FR', 'DE', 'IT', 'ES', 'AT', 'PL', 'NO', 'US', 'CZ', 'SK', 'RU', 'BR', 'SE', 'CN', 'CH', 'TR', 'AU', 'TW');
"
#hive -e "$sql"
spark-sql \
