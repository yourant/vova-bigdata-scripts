...
#finance gmv
sql="
DROP TABLE IF EXISTS tmp.tmp_zyzheng_refund_time;
CREATE TABLE tmp.tmp_zyzheng_refund_time AS
select
sol.order_goods_id,
max(sol.create_time) AS refund_time
from
ods.vova_sku_ops_log sol
where sol.ops='sku_pay_status' AND sol.status=4
group by sol.order_goods_id
;

INSERT OVERWRITE TABLE rpt.rpt_finance_gmv PARTITION (pt = '${cur_date}')
select
concat(trunc('${cur_date}','MM'),'---', '2021-03-19') AS event_date,
...
LEFT JOIN dwd.dim_order_goods dog2 on dog2.parent_rec_id = dog.order_goods_id AND dog2.parent_rec_id > 0
WHERE dog.pay_status = 2
AND date(dog.receive_time) >= trunc('${cur_date}','MM')
AND date(dog.receive_time) <= last_day('${cur_date}')
AND date(dog.receive_time) <= '2021-03-19'
...
from dwd.dim_order_goods dog
LEFT JOIN dwd.dim_order_goods dog2 on dog2.parent_rec_id = dog.order_goods_id AND dog2.parent_rec_id > 0
INNER JOIN ods.vova_adyen_klarna_capture_record kcr on kcr.order_sn = dog.order_sn
...
AND date(kcr.capture_time) <= '2021-03-19'
AND dog.parent_rec_id = 0
...
from
dwd.fact_refund rr
INNER JOIN dwd.dim_order_goods dog on rr.order_goods_id = dog.order_goods_id
INNER JOIN tmp.tmp_zyzheng_refund_time tt on tt.order_goods_id = dog.order_goods_id
where dog.sku_pay_status=4
AND date(tt.refund_time) >= trunc('${cur_date}','MM')
AND date(tt.refund_time) <= last_day('${cur_date}')
AND date(tt.refund_time) <= '2021-03-19'
...
FROM dwd.fact_act_ord_gs fag
INNER JOIN dwd.dim_payment dp ON dp.payment_id = fag.pmt_id
INNER JOIN dwd.dim_goods dg ON dg.goods_id = fag.gs_id
WHERE fag.pay_sts = 2
  AND fag.ord_id = 0
  AND fag.rcv_time >= trunc('${cur_date}','MM')
  AND fag.rcv_time <= last_day('${cur_date}')
  AND date(fag.rcv_time) <= '2021-03-19'
GROUP BY CUBE (date(fag.rcv_time),dp.payment_name,dg.mct_id)
) receive_data
GROUP BY event_date,source,payment_name,mct_id
) final_data
LEFT JOIN dwd.dim_merchant dm on dm.mct_id = final_data.mct_id
