...
SELECT
    /*+ REPARTITION(200) */
       ...
       case when vaptc.expected_period = 0 then '单日单次' when vaptc.expected_period = 20 then '每日循环' else 'others' end job_rate
FROM ods_vova_ext.ods_vova_app_push_logs vapl
         inner join ods_vova_vtp.ods_vova_app_push_task_config vaptc on vaptc.id = vapl.task_config_id
         inner join ods_vova_vtp.ods_vova_app_push_task vapt2 on vapt2.id = vapl.task_id
         left join dim.dim_vova_devices dev on vapl.user_id = dev.current_buyer_id
         left join (select vut.user_id                            as buyer_id,
                    ...
                    group by vut.user_id) as ug on ug.buyer_id = vapl.user_id
...
from tmp.tmp_push_logs_${table_suffix} tpl
...
select /*+ REPARTITION(3) */ distinct nvl(fpc.datasource, 'NA')        AS datasource,
                                      dev.region_code,
...
from dwd.dwd_vova_fact_push_click fpc
         left join dim.dim_vova_devices dev on dev.device_id = fpc.device_id
         join (select device_id
               from dwd.dwd_vova_log_common_click a
                        join ods_vova_vtp.ods_vova_app_push_task b
                             on split(a.element_id, '@')[0] = b.id
               where a.pt = '${cur_date}'
                 and a.element_name = 'PushUserDecide'
                 and a.element_id like '%@%'
               group by device_id) flcc on fpc.device_id = flcc.device_id
         left join ods_vova_vtp.ods_vova_app_push_task_config vaptc on fpc.config_id = vaptc.id
...
select /*+ REPARTITION(10) */ fpc.device_id,
       gi.buyer_id,
       gi.pd_dvce
from tmp.tmp_push_click_all_${table_suffix} as fpc
         inner join
     (select gi.device_id,
             gi.buyer_id,
             gi.collector_tstamp,
     ...
     ) gi on fpc.device_id = gi.device_id
...
select /*+ REPARTITION(20) */ fpc.device_id,
       gi.buyer_id,
       gi.ex_pd_dvce
from tmp.tmp_push_click_all_${table_suffix} as fpc
         inner join
...
select /*+ REPARTITION(20) */ fpc.device_id
from tmp.tmp_push_click_all_${table_suffix} fpc
         inner join
     (select cc.device_id,
             cc.collector_tstamp
     ...
     ) cc on fpc.device_id = cc.device_id
...
select /*+ REPARTITION(20) */ fpc.device_id,
       ...
       og.buyer_id
from tmp.tmp_push_click_all_${table_suffix} fpc
         inner join dim.dim_vova_order_goods og on fpc.device_id = og.device_id
...
from tmp.tmp_push_click_all_${table_suffix} fpc
         inner join dwd.dwd_vova_fact_pay py on fpc.device_id = py.device_id
         inner join dim.dim_vova_goods dg on py.goods_id = dg.goods_id
...
select /*+ REPARTITION(10) */ nvl(fpc.datasource, 'NA') AS datasource,
       fpc.region_code,
       fpc.platform,
       fpc.target_type,
       fpc.r_tag,
...
CREATE TABLE IF NOT EXISTS tmp.push_click_behavior_v2_result_01_${table_suffix} as
 select /*+ REPARTITION(10) */ ps.datasource,
                ps.platform,
                ps.region_code,
                ...
                ps.push_num,
                ps.success_num
         from tmp.push_stas_${table_suffix} as ps
         ...
         select 'vova'                        as datasource,
                ...
                0                             as push_num,
                0                             as success_num
         from tmp.tmp_push_click_${table_suffix} fpc
         where datasource = 'vova'
         ...
         select 'vova'                       as datasource,
                ...
                0                            as push_num,
                0                            as success_num
         from tmp.tmp_push_click_${table_suffix} as fpc
                  inner join tmp.tmp_push_click_all_imp_${table_suffix} gi on fpc.device_id = gi.device_id
         where datasource = 'vova'
         group by cube (fpc.platform,  fpc.config_id, fpc.main_channel, fpc.region_code,fpc.job_rate)
         union all
         select 'vova'                        as datasource,
                ...
                0                             as success_num
         from tmp.tmp_push_click_${table_suffix} as fpc
                  inner join tmp.tmp_push_click_all_pd_${table_suffix} gi on fpc.device_id = gi.device_id
         where datasource = 'vova'
         group by cube (fpc.platform, fpc.config_id, fpc.main_channel, fpc.region_code,fpc.job_rate)
		 union all
		 select 'vova'                       as datasource,
                ...
                0                            as success_num
         from tmp.tmp_push_click_${table_suffix} as fpc
                  inner join (select device_id from tmp.tmp_push_click_all_pd_${table_suffix} union all select device_id from tmp.tmp_push_click_all_imp_${table_suffix}) gi on fpc.device_id = gi.device_id
         where datasource = 'vova'
         group by cube (fpc.platform,  fpc.config_id, fpc.main_channel, fpc.region_code,fpc.job_rate)
         ...
         select 'vova'                       as datasource,
                ...
                0                            as success_num
         from tmp.tmp_push_click_${table_suffix} fpc
                  inner join tmp.tmp_push_click_all_clk_${table_suffix} cc on fpc.device_id = cc.device_id
         where datasource = 'vova'
         group by cube (fpc.platform, fpc.config_id, fpc.main_channel, fpc.region_code,fpc.job_rate)
         union all
         select 'vova'                       as datasource,
                ...
                0                            as success_num
         from tmp.tmp_push_click_${table_suffix} fpc
                  inner join tmp.tmp_push_click_all_order_${table_suffix} og on fpc.device_id = og.device_id
         where datasource = 'vova'
         group by cube (fpc.platform,fpc.config_id, fpc.main_channel, fpc.region_code,fpc.job_rate)
         union all
         select 'vova'                                                 as datasource,
                ...
                0                                                      as success_num
         from tmp.tmp_push_click_${table_suffix} fpc
                  inner join tmp.tmp_push_click_all_pay_${table_suffix} py on fpc.device_id = py.device_id
         where datasource = 'vova'
         group by cube (fpc.platform, fpc.config_id, fpc.main_channel, fpc.region_code,fpc.job_rate)
;

CREATE TABLE IF NOT EXISTS tmp.push_click_behavior_v2_result_02_${table_suffix} as
select /*+ REPARTITION(10) */ tmp.datasource,
       ...
       tmp.job_rate
from tmp.push_click_behavior_v2_result_01_${table_suffix} tmp
...
select /*+ REPARTITION(10) */ tmp.datasource,
       ...
       sum(tmp.success_num)          as success_num,
       tmp.job_rate
from tmp.push_click_behavior_v2_result_03_${table_suffix} tmp
where tmp.datasource not IN ('NA', 'all')
  and tmp.platform in ('android', 'ios', 'all')
  and tmp.target_type != 'all'
  and tmp.config_id not IN ('NA', 'all')
...
select /*+ REPARTITION(10) */ datasource,
       push_date,
       platform,
       region_code,
       click_interval,
       ...
       brand_gmv,
       no_brand_gmv
from tmp.push_click_behavior_v2_result_02_${table_suffix}
union all
select /*+ REPARTITION(10) */ datasource,
       push_date,
       platform,
       ...
       brand_gmv,
       no_brand_gmv
from tmp.push_click_behavior_v2_result_04_${table_suffix}
;
