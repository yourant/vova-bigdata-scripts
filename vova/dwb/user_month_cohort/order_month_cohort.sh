...
/*+ REPARTITION(1) */
       ...
       nvl(final.buyer_type, 'all')      AS buyer_type,
       nvl(final.first_pay_month, 'all') AS pt
FROM (
         SELECT temp_devices.device_id AS next_0,
                if(months_between(temp2.start_month, temp_devices.first_pay_month) = 1, temp_devices.device_id,
                   NULL) AS next_1,
                if(months_between(temp2.start_month, temp_devices.first_pay_month) = 2, temp_devices.device_id,
                   NULL) AS next_2,
                if(months_between(temp2.start_month, temp_devices.first_pay_month) = 3, temp_devices.device_id,
                   NULL) AS next_3,
                if(months_between(temp2.start_month, temp_devices.first_pay_month) = 4, temp_devices.device_id,
                   NULL) AS next_4,
                if(months_between(temp2.start_month, temp_devices.first_pay_month) = 5, temp_devices.device_id,
                   NULL) AS next_5,
                if(months_between(temp2.start_month, temp_devices.first_pay_month) = 6, temp_devices.device_id,
                   NULL) AS next_6,
                if(months_between(temp2.start_month, temp_devices.first_pay_month) = 7, temp_devices.device_id,
                   NULL) AS next_7,
                if(months_between(temp2.start_month, temp_devices.first_pay_month) = 8, temp_devices.device_id,
                   NULL) AS next_8,
                if(months_between(temp2.start_month, temp_devices.first_pay_month) = 9, temp_devices.device_id,
                   NULL) AS next_9,
                if(months_between(temp2.start_month, temp_devices.first_pay_month) = 10, temp_devices.device_id,
                   NULL) AS next_10,
                if(months_between(temp2.start_month, temp_devices.first_pay_month) = 11, temp_devices.device_id,
                   NULL) AS next_11,
                if(months_between(temp2.start_month, temp_devices.first_pay_month) = 12, temp_devices.device_id,
                   NULL) AS next_12,
         ...
         FROM (SELECT trunc(tdfp.first_pay_time, 'MM') AS first_pay_month,
                      tdfp.device_id,
               ...
               GROUP BY trunc(tdfp.first_pay_time, 'MM'), tdfp.device_id, nvl(dd.platform, 'NA'), nvl(dd.region_code, 'NALL'), nvl(tdfp.datasource, 'NA')) temp_devices
               LEFT JOIN
              (SELECT trunc(pt, 'MM') AS start_month,
                      su.device_id,
                      su.datasource
               FROM dwd.dwd_vova_fact_start_up su
               GROUP BY trunc(pt, 'MM'), su.device_id, su.datasource) temp2
              ...
              LEFT JOIN ( -- buyer_type: 前置仓直发
                select
                device_id,
                datasource,
                start_month,
                'only_pre_direct' buyer_type -- 只购买前置仓
                from tmp.tmp_order_goods_cnt
                      where pre_direct_cnt = order_cnt and pre_direct_cnt>0
                union all
                select
                device_id,
                datasource,
                start_month,
                case when pre_direct_cnt >0 and order_cnt >0 then 'include_pre_direct'
                else 'no_pre_direct' end buyer_type
                from tmp.tmp_order_goods_cnt
                ) tmp_pre_direct
              ON tmp_pre_direct.device_id = temp_devices.device_id
                AND tmp_pre_direct.datasource = temp_devices.datasource
                and tmp_pre_direct.start_month = temp_devices.first_pay_month
         ) final
GROUP BY CUBE(final.first_pay_month, final.platform, final.region_code, final.datasource, final.buyer_type)
...
FROM (
    SELECT if(months_between(temp2.pay_month, temp1.pay_month) = 0, temp1.device_id, NULL) AS next_0,
             ...
             nvl(tmp_pre_direct.buyer_type, 'no_pre_direct') AS buyer_type
      FROM (SELECT trunc(pay_time, 'MM') AS pay_month,
                   fp.device_id,
                   fp.region_code,
                   fp.platform,
                   fp.datasource
           ...
           LEFT JOIN
             (SELECT trunc(first_pay_time, 'MM') AS first_pay_month,
                     tdfp.device_id,
                     tdfp.datasource
              FROM tmp.tmp_vova_device_first_pay tdfp
              WHERE first_pay_time IS NOT NULL
              GROUP BY trunc(first_pay_time, 'MM'), tdfp.device_id, tdfp.datasource) temp_first_pay
           ...
           ON tmp_pre_direct.device_id = temp1.device_id
               AND tmp_pre_direct.datasource = temp1.datasource
               and tmp_pre_direct.start_month = temp1.pay_month
           INNER JOIN
             (SELECT trunc(pay_time, 'MM') AS pay_month,
              ...
              GROUP BY trunc(pay_time, 'MM'), fp2.device_id, fp2.region_code, fp2.platform, fp2.datasource) temp2
...
GROUP BY CUBE (final.pay_month, final.is_new_user, final.platform, final.region_code, final.datasource, final.buyer_type)
HAVING pay_month != 'all';
"
#如果使用spark-sql运行，则执行spark-sql --conf "spark.sql.parquet.writeLegacyFormat=true" -e
spark-sql \
