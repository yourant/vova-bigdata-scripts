...
hive -e "msck repair table mlb.mlb_vova_rec_m_nurecallad_d;"
if [ $? -ne 0 ];then
  exit 1
fi

cnt=$(spark-sql -e "select count(*) from mlb.mlb_vova_rec_m_nurecallad_d where pt ='${pre_date}';" |tail -1)
if [ ${cnt} -le 0 ];then
...
sql="
drop table if exists rec_recall.ads_rec_m_nurecallad_d_pre;
drop table if exists rec_recall.ads_rec_m_nurecallad_d_new;
create table rec_recall.ads_rec_m_nurecallad_d_new(
...
create table if not exists rec_recall.ads_rec_m_nurecallad_d (
\`id\`           int(11) NOT NULL AUTO_INCREMENT COMMENT '自增主键',
\`cluster_key\`  varchar(100)  NOT NULL COMMENT '分组标识',
\`goods_id\`     int(11)    NOT NULL COMMENT '商品id',
\`rank_num\`     int(11)    NOT NULL COMMENT '综合排序topN的rank索引',
...
--m 1 \
--table ads_rec_m_nurecallad_d_new \
--hcatalog-database mlb \
--hcatalog-table mlb_vova_rec_m_nurecallad_d \
...
rename table rec_recall.ads_rec_m_nurecallad_d to rec_recall.ads_rec_m_nurecallad_d_pre,rec_recall.ads_rec_m_nurecallad_d_new to rec_recall.ads_rec_m_nurecallad_d;
EOF
echo "-------rename结束--------"
