...
if [ ! -n "${table_name}" ]; then
  echo "Error: 任务: mlb_sqoop_k2i, freedoms 中没有配置 table_name！！！"
  exit 1
fi
...
create table rec_recall.${table_name}_new(
  ...
  \`key_id\`          bigint         NOT NULL COMMENT '商品id',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=DYNAMIC COMMENT='k2i 召回结果表';
...
--columns key_id,rec_goods_list,score_list,model_name \
