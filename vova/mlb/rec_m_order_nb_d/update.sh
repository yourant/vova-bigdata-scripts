...
order_i2i_num=`aws s3 ls s3://vova-mlb/REC/data/match/match_result/order_i2i/no_brand_serial/pt=${pre_date}/ | wc -l`
if [ ${order_i2i_num} -eq 0 ]; then
  echo "pt=${pre_date} order_i2i num = 0"
  exit 1
fi

hive -e "msck repair table mlb.mlb_vova_rec_m_order_u2i_nb_d;"
hive -e "msck repair table mlb.mlb_vova_rec_m_order_i2i_nb_d;"
...
mysql -h rec-bi.cluster-cznqgcwo1pjt.us-east-1.rds.amazonaws.com -u bdwriter -pDd7LvXRPDP4iIJ7FfT8e <<EOF
drop table if exists themis.rec_m_order_i2i_nb_d_new;
drop table if exists themis.rec_m_order_i2i_nb_d_pre;
CREATE TABLE themis.rec_m_order_i2i_nb_d_new
(
    goods_id      bigint COMMENT 'buyer_id',
    rec_goods_id_list      varchar(3072) COMMENT 'rec_goods_id_list',
    score_list    varchar(3072) COMMENT 'score_list',
    cur_date TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0)  COMMENT '日期',
    PRIMARY KEY (goods_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='rec_m_order_i2i_nb_d';
EOF
...
--connect jdbc:mysql://rec-bi.cluster-cznqgcwo1pjt.us-east-1.rds.amazonaws.com:3306/themis \
--username bdwriter --password Dd7LvXRPDP4iIJ7FfT8e \
--table rec_m_order_i2i_nb_d_new \
--m 1 \
--hcatalog-database mlb \
--hcatalog-table mlb_vova_rec_m_order_i2i_nb_d \
--hcatalog-partition-keys pt \
--hcatalog-partition-values ${pre_date} \
...
mysql -h rec-bi.cluster-cznqgcwo1pjt.us-east-1.rds.amazonaws.com -u bdwriter -pDd7LvXRPDP4iIJ7FfT8e <<EOF
rename table themis.rec_m_order_i2i_nb_d to themis.rec_m_order_i2i_nb_d_pre;
rename table themis.rec_m_order_i2i_nb_d_new to themis.rec_m_order_i2i_nb_d;
EOF
