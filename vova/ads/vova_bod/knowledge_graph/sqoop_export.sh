...
m_sql="
drop table if exists rec_recall.ads_vova_kg_d_pre;
drop table if exists rec_recall.ads_vova_kg_d_new;
CREATE TABLE IF NOT EXISTS \`rec_recall\`.\`ads_vova_kg_d_new\` (
...
CREATE TABLE IF NOT EXISTS \`rec_recall\`.\`ads_vova_kg_d\` (
\`id\` int(11) NOT NULL AUTO_INCREMENT,
\`buyer_id\` int(11) NOT NULL COMMENT '用户id',
\`bod_id_list\` mediumtext NOT NULL COMMENT '榜单id列表',
\`score_list\` mediumtext NOT NULL COMMENT '分数列表',
...
RENAME table rec_recall.ads_vova_kg_d to rec_recall.ads_vova_kg_d_pre,rec_recall.ads_vova_kg_d_new to rec_recall.ads_vova_kg_d;
EOF
echo "-------rename结束--------"
if [ $? -ne 0 ];then
  exit 1
