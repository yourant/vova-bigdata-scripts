...
left join dim.dim_vova_goods g on og.goods_id = g.goods_id
...
from
(
select geo_country,virtual_goods_id, device_id click_device_id,null impression_device_id,1 clicks,0 impressions from dwd.dwd_vova_log_goods_click where pt>='$pre_w' and platform='mob' and datasource='vova' and geo_country in ('GB','FR','DE','IT','ES')
union all
select geo_country,virtual_goods_id,null click_device_id,device_id impression_device_id,0 clicks,1 impressions from dwd.dwd_vova_log_goods_impression where pt>='$pre_w' and platform='mob' and datasource='vova' and geo_country in ('GB','FR','DE','IT','ES')
) t
join dim.dim_vova_goods g on g.virtual_goods_id = t.virtual_goods_id
join (select distinct goods_id from ads_test_cat_gmv_1w) t1 on t1.goods_id =g.goods_id
where g.is_on_sale = 1
group by g.goods_id,g.first_cat_id,g.second_cat_id,t.geo_country,if(g.brand_id>0,1,0)
),
ads_test_cat_gcr_1w as
(
select
t1.goods_id,
t1.is_brand,
...
from ads_test_cat_gmv_1w t1
left join ads_test_cat_ctr_1w t2 on t1.goods_id = t2.goods_id and t1.geo_country = t2.geo_country
where t2.impressions>=500
)
insert overwrite table  ads.ads_test_cat_gcr_res_w
select
/*+ REPARTITION(1) */
first_cat_id,
...
from ads_test_cat_gcr_1w
) t where rank =1
) t1;
"
spark-sql  --conf "spark.app.name=ads_cat_gcr_w" --conf "spark.dynamicAllocation.maxExecutors=100"  -e "$sql"
#如果脚本失败，则报错
if [ $? -ne 0 ];then
  exit 1
fi
...
--hcatalog-table ads_test_cat_gcr_res_w \
