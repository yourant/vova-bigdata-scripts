...
#默认日期为昨天
if [ ! -n "$1" ]; then
  pre_hour=`date -d "-168hour" "+%Y-%m-%d %H:%M:%S"`
fi
echo "$pre_hour "
#默认小时
pt=$2
...
where pt>='$pt' and collector_tstamp >='$pre_hour' and event_type='goods' and page_code ='search_result' and list_type in ('/search_result_recommend','/search_result_sold','/search_result_price_desc','/search_result_price_asc','/search_result')
