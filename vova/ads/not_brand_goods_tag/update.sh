...
join tmp_goods tmp on a.goods_id=tmp.goods_id
...
select
	...
	where t.cnt>=3 and round(t.good_cnt/t.cnt,2) > 0.7
UNION
select
...
join tmp_goods tmp on fp.goods_id=tmp.goods_id
...
join tmp_goods tmp on a.goods_id=tmp.goods_id
 ...
 and b.is_on_sale=1
"
spark-sql --conf "spark.app.name=ads_vova_not_brand_goods_tag_data" --conf "spark.dynamicAllocation.maxExecutors=150" -e "$sql"
#如果脚本失败，则报错
if [ $? -ne 0 ];then
  exit 1
fi