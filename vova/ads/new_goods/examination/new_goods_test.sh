...
INSERT OVERWRITE TABLE ads.ads_vova_goods_test_collector_data PARTITION (pt)
select
/*+ REPARTITION(5) */
goods_id,
original_name,
...
FROM dwd.dwd_vova_log_impressions_arc log
         ...
         INNER JOIN ads.ads_vova_goods_examination_poll test_poll on test_poll.goods_id = dg.goods_id
...
INSERT OVERWRITE TABLE ads.ads_vova_new_goods_examination_behave
select
/*+ REPARTITION(1) */
'vova' as datasource,
goods_id,
...
count(distinct tcd.device_id) as impressions_uv,
0 AS clicks,
0 AS clicks_uv,
0 AS add_cart_cnt,
0 as gmv,
...
inner join ads.ads_vova_goods_test_collector_data tcd on tcd.goods_id = poll.goods_id
where tcd.collector_tstamp >= poll.add_test_time
and tcd.original_name = 'goods_impression'
and tcd.datasource = 'vova'
and tcd.platform = 'mob'
...
0 AS impressions_uv,
count(*) as clicks,
count(distinct tcd.device_id) as clicks_uv,
0 AS add_cart_cnt,
0 as gmv,
0 as sales_order
from
...
FROM ods_vova_vts.ods_vova_order_info_h oi
         ...
         inner join ads.ads_vova_goods_examination_poll poll on poll.goods_id = og.goods_id
WHERE oi.pay_status >= 1
  AND oi.from_domain LIKE '%api.vova%'
...
DROP TABLE IF EXISTS tmp.tmp_ads_vova_new_goods_examination_pre;
CREATE TABLE tmp.tmp_ads_vova_new_goods_examination_pre
SELECT
datasource,
goods_id,
impressions,
...
from
ads.ads_vova_goods_examination_poll test_poll
LEFT JOIN ads.ads_vova_new_goods_examination_behave b1 on test_poll.goods_id = b1.goods_id
) t1
;

DROP TABLE IF EXISTS tmp.tmp_ads_vova_new_goods_examination_diff_1;
CREATE TABLE tmp.tmp_ads_vova_new_goods_examination_diff_1
SELECT
fin.datasource,
fin.goods_id,
fin.impressions,
...
fin.add_test_time
from
ads.ads_vova_new_goods_examination_summary fin
inner join tmp.tmp_ads_vova_new_goods_examination_pre pre on pre.goods_id = fin.goods_id
where fin.test_goods_result_status in (3,5,6,7) and fin.add_test_time = pre.add_test_time
;

DROP TABLE IF EXISTS tmp.tmp_ads_vova_new_goods_examination_not_change;
CREATE TABLE tmp.tmp_ads_vova_new_goods_examination_not_change
SELECT
fin.goods_id,
fin.status_change_time
from
tmp.tmp_ads_vova_new_goods_examination_pre pre
inner join ads.ads_vova_new_goods_examination_summary fin on pre.goods_id = fin.goods_id
where fin.test_goods_result_status not in (3,5,6,7) and pre.test_goods_result_status = fin.test_goods_result_status
;


INSERT OVERWRITE TABLE ads.ads_vova_new_goods_examination_summary
select
/*+ REPARTITION(1) */
pre.datasource,
pre.goods_id,
...
from
tmp.tmp_ads_vova_new_goods_examination_pre pre
left join tmp.tmp_ads_vova_new_goods_examination_diff_1 d1 on d1.goods_id = pre.goods_id
left join tmp.tmp_ads_vova_new_goods_examination_not_change dc on dc.goods_id = pre.goods_id
...
d1.add_test_time
from
tmp.tmp_ads_vova_new_goods_examination_diff_1 d1
;


INSERT OVERWRITE TABLE ads.ads_vova_new_goods_examination_summary_history PARTITION (pt = '${cur_date}')
SELECT
/*+ REPARTITION(1) */
datasource,
goods_id,
...
add_test_time
from
ads.ads_vova_new_goods_examination_summary
;

INSERT OVERWRITE TABLE ads.ads_vova_new_goods_examination_summary_history_export PARTITION (pt = '${cur_date}')
SELECT
/*+ REPARTITION(1) */
ges.datasource,
ges.goods_id,
...
ges.gmv_cr,
ges.goods_score,
nvl(agp.gcr_1w, 0) AS gcr_1w,
nvl(agp.gmv_cr_1w, 0) AS gmv_cr_1w,
nvl(agp.impressions_1w, 0) AS impressions_1w,
...
from
ads.ads_vova_new_goods_examination_summary ges
LEFT JOIN ads.ads_vova_goods_examination_poll poll on poll.goods_id = ges.goods_id
left join dim.dim_vova_goods dg on dg.goods_id = ges.goods_id
left join (
select
agp.goods_id,
agp.gcr as gcr_1w,
nvl(agp.gmv / agp.impressions * 10000, 0) as gmv_cr_1w,
agp.impressions as impressions_1w
FROM ads.ads_vova_goods_performance agp
inner join (select max(pt) as max_pt from ads.ads_vova_goods_performance agp) max on max.max_pt = agp.pt
WHERE agp.datasource = 'vova'
  AND agp.platform = 'mob'
  AND agp.region_code = 'all'
) agp on agp.goods_id = ges.goods_id
;

INSERT OVERWRITE TABLE ads.ads_vova_new_goods_examination_summary_display
select
/*+ REPARTITION(1) */
datasource,
goods_id,
...
goods_source_text,
case
when concat(goods_source_image,goods_source_basic,goods_source_text) = '100'
then 'a'
when concat(goods_source_image,goods_source_basic,goods_source_text) = '010'
then 'b'
when concat(goods_source_image,goods_source_basic,goods_source_text) = '001'
then 'c'
when concat(goods_source_image,goods_source_basic,goods_source_text) = '110'
then 'ab'
when concat(goods_source_image,goods_source_basic,goods_source_text) = '101'
then 'ac'
when concat(goods_source_image,goods_source_basic,goods_source_text) = '011'
then 'bc'
when concat(goods_source_image,goods_source_basic,goods_source_text) = '111'
then 'abc'
else 'NA' end as rec_original
from
ads.ads_vova_new_goods_examination_summary_history_export
where pt = '${cur_date}'
;

INSERT OVERWRITE TABLE dwb.dwb_vova_new_goods_examination PARTITION (pt = '${cur_date}')
select
/*+ REPARTITION(1) */
...
--conf "spark.app.name=vova_new_goods_examination_h" \
