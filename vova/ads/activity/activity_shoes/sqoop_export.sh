...
hive -e "msck repair table ads.ads_vova_activity_shoes;"
if [ $? -ne 0 ];then
  exit 1
fi
echo "msck repair table ok"

cnt=$(spark-sql -e "select count(*) from ads.ads_vova_activity_shoes where pt ='${pre_date}';" | tail -1)
echo ${cnt}
if [ ${cnt} -le 0 ];then
...
sql="
drop table if exists themis.ads_vova_activity_shoes_pre;
drop table if exists themis.ads_vova_activity_shoes_new;
CREATE TABLE IF NOT EXISTS \`themis\`.\`ads_vova_activity_shoes_new\` (
...
CREATE TABLE IF NOT EXISTS \`themis\`.\`ads_vova_activity_shoes\` (
  \`id\` int(11) NOT NULL AUTO_INCREMENT,
  \`goods_id\` int(11) NOT NULL COMMENT '商品id',
  \`region_id\` int(11) NOT NULL COMMENT '国家id',
  \`first_cat_id\` int(11) NOT NULL COMMENT '一级分类id',
...
--m 1 \
--table ads_vova_activity_shoes_new \
--hcatalog-database ads \
--hcatalog-table ads_vova_activity_shoes \
...
rename table themis.ads_vova_activity_shoes to themis.ads_vova_activity_shoes_pre,themis.ads_vova_activity_shoes_new to themis.ads_vova_activity_shoes;
EOF
echo "-------rename结束--------"
