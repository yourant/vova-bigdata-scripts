...
sh /mnt/vova-bigdata-scripts/common/table_check.sh  ads.ads_vova_activity_new_user 800 "pt='${pre_date}'"


sql="
drop table if exists themis.ads_activity_new_user_pre;
drop table if exists themis.ads_activity_new_user_new;
CREATE TABLE IF NOT EXISTS \`themis\`.\`ads_activity_new_user_new\` (
...
CREATE TABLE IF NOT EXISTS \`themis\`.\`ads_activity_new_user\` (
  \`id\` int(11) NOT NULL AUTO_INCREMENT,
  \`goods_id\` int(11) NOT NULL COMMENT '商品id',
  \`region_id\` int(11) NOT NULL COMMENT '国家id',
  \`first_cat_id\` int(11) NOT NULL COMMENT '一级分类id',
...
--m 1 \
--table ads_activity_new_user_new \
--hcatalog-database ads \
--hcatalog-table ads_vova_activity_new_user \
...
rename table themis.ads_activity_new_user to themis.ads_activity_new_user_pre,themis.ads_activity_new_user_new to themis.ads_activity_new_user;
EOF
echo "-------rename结束--------"
