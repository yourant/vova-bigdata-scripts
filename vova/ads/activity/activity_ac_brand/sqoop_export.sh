...
sh /mnt/vova-bigdata-scripts/common/table_check.sh  ads.ads_vova_activity_ac_brand 6000 "pt='${pre_date}'"
...
sql="
drop table if exists themis.ads_activity_ac_brand_pre;
drop table if exists themis.ads_activity_ac_brand_new;
CREATE TABLE IF NOT EXISTS \`themis\`.\`ads_activity_ac_brand_new\` (
...
CREATE TABLE IF NOT EXISTS \`themis\`.\`ads_activity_ac_brand\` (
  \`id\` int(11) NOT NULL AUTO_INCREMENT,
  \`goods_id\` int(11) NOT NULL COMMENT '商品id',
  \`region_id\` int(11) NOT NULL COMMENT '国家id',
  \`first_cat_id\` int(11) NOT NULL COMMENT '一级分类id',
...
--m 1 \
--table ads_activity_ac_brand_new \
--hcatalog-database ads \
--hcatalog-table ads_vova_activity_ac_brand \
...
rename table themis.ads_activity_ac_brand to themis.ads_activity_ac_brand_pre,themis.ads_activity_ac_brand_new to themis.ads_activity_ac_brand;
EOF
echo "-------rename结束--------"
