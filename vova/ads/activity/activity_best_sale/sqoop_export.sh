...
sh /mnt/vova-bigdata-scripts/common/table_check.sh  ads.ads_vova_activity_best_sale 3000 "pt='${pre_date}'"
...
sql="
drop table if exists themis.ads_activity_best_sale_pre;
drop table if exists themis.ads_activity_best_sale_new;
CREATE TABLE IF NOT EXISTS \`themis\`.\`ads_activity_best_sale_new\` (
...
CREATE TABLE IF NOT EXISTS \`themis\`.\`ads_activity_best_sale\` (
  \`id\` int(11) NOT NULL AUTO_INCREMENT,
  \`goods_id\` int(11) NOT NULL COMMENT '商品id',
  \`region_id\` int(11) NOT NULL COMMENT '国家id',
  \`first_cat_id\` int(11) NOT NULL COMMENT '一级分类id',
...
--m 1 \
--table ads_activity_best_sale_new \
--hcatalog-database ads \
--hcatalog-table ads_vova_activity_best_sale \
...
rename table themis.ads_activity_best_sale to themis.ads_activity_best_sale_pre,themis.ads_activity_best_sale_new to themis.ads_activity_best_sale;
EOF
echo "-------rename结束--------"
