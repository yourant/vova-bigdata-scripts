...
# score 计算
job4_name="ads_vova_buyer_portrait_first_category_likes_exp"

###逻辑sql
sql="
insert overwrite table ads.ads_vova_buyer_portrait_first_category_likes partition(pt='${cur_date}')
SELECT
  /*+ REPARTITION(10) */
  buyer_id,
  first_cat_id,
...
(
  ...
  WHERE
    pt > date_sub('${cur_date}', 30) AND pt <= '${cur_date}' and first_cat_id is not null
  GROUP BY
    buyer_id,
    first_cat_id,
    pt
...
insert overwrite table ads.ads_vova_buyer_portrait_first_category_likes_medium_term partition(pt='${cur_date}')
SELECT
  /*+ REPARTITION(10) */
  buyer_id,
  first_cat_id,
...
(
  ...
  FROM
    dws.dws_vova_buyer_goods_behave
  WHERE
    pt >= date_sub('${cur_date}', 60) AND pt < date_sub('${cur_date}', 15) and first_cat_id is not null
  GROUP BY
    buyer_id,
    first_cat_id,
    pt
...
insert overwrite table ads.ads_vova_buyer_portrait_first_category_likes_long_term partition(pt='${cur_date}')
select
/*+ REPARTITION(10) */
...
where fp.confirm_time < date_sub('${cur_date}', 60) and dg.first_cat_id is not null
group by
  fp.buyer_id,
  dg.first_cat_id
;

insert overwrite table ads.ads_vova_buyer_portrait_first_category_likes_exp partition(pt='${cur_date}')
select
/*+ REPARTITION(10) */
  buyer_id,
  first_cat_id,
  weights,
  log(2,if(weights_short*1000>127,127,weights_short*1000)+1)/7,
  log(2,if(weights_medium*1000>127,127,weights_medium*1000)+1)/7,
  log(18,if(weights_long*1000>323,323,weights_long*1000)+1)/2,
  log(22,if((0.6 * weights_short + 0.3 * weights_medium + 0.1 * weights_long)*10000>483,483,(0.6 * weights_short + 0.3 * weights_medium + 0.1 * weights_long)*10000)+1)/2 -- 综合偏好
from
(
  select
    buyer_id,
    first_cat_id,
    nvl(first(weights),0) weights, -- 偏好度
  ...
  (
    select
      r.buyer_id,
      l.first_cat_id,
      exp(- 0.0767 * nvl(l.min_clk_day_gap, 31)) * (0.5 * ( ord_cnt_1m / ( ord_cnt_all + 1 ) )
    ...
    (
      ...
      from ads.ads_vova_buyer_portrait_first_category_likes
  ...
  union all
    ...
    (
      ...
      from ads.ads_vova_buyer_portrait_first_category_likes_medium_term
      where pt='${cur_date}' and min_clk_day_gap is not null
      GROUP by buyer_id
    ) r
    left join
  ...
  union all
    ...
    (
      ...
      from ads.ads_vova_buyer_portrait_first_category_likes_long_term
    ...
    on l.buyer_id = r.buyer_id
  ) tmp where buyer_id > 0 and first_cat_id is not null
    group by buyer_id, first_cat_id
) result
;
"
