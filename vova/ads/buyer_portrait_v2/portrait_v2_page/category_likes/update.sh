...
insert overwrite table ads.ads_vova_buyer_page_category_top_behave partition(bpt)
select
/*+ REPARTITION(1) */
tmp1.buyer_id,
db.datasource,
...
'click' as type,
7 as day_gap,
collect_list(concat(cat_name,':',clk_cnt_1w)) as behave_top_array
from
(select
cl.buyer_id,
dc.cat_name,
...
from
ads.ads_vova_buyer_portrait_category_likes cl
left join dim.dim_vova_category dc on cl.cat_id = dc.cat_id
where cl.pt='${pre_date}' and cl.clk_cnt_1w>0)
where rk <=20
group by
buyer_id
...
'click' as type,
15 as day_gap,
collect_list(concat(cat_name,':',clk_cnt_15d)) as behave_top_array
from
(select
cl.buyer_id,
dc.cat_name,
...
'click' as type,
30 as day_gap,
collect_list(concat(cat_name,':',clk_cnt_1m)) as behave_top_array
from
(select
cl.buyer_id,
dc.cat_name,
...
'add_cat' as type,
7 as day_gap,
collect_list(concat(cat_name,':',add_cat_cnt_1w)) as behave_top_array
from
(select
cl.buyer_id,
dc.cat_name,
...
'add_cat' as type,
15 as day_gap,
collect_list(concat(cat_name,':',add_cat_cnt_15d)) as behave_top_array
from
(select
cl.buyer_id,
dc.cat_name,
...
'add_cat' as type,
30 as day_gap,
collect_list(concat(cat_name,':',add_cat_cnt_1m)) as behave_top_array
from
(select
cl.buyer_id,
dc.cat_name,
...
'order' as type,
7 as day_gap,
collect_list(concat(cat_name,':',ord_cnt_1w)) as behave_top_array
from
(select
cl.buyer_id,
dc.cat_name,
...
'order' as type,
15 as day_gap,
collect_list(concat(cat_name,':',ord_cnt_15d)) as behave_top_array
from
(select
cl.buyer_id,
dc.cat_name,
...
'order' as type,
30 as day_gap,
collect_list(concat(cat_name,':',ord_cnt_1m)) as behave_top_array
from
(select
cl.buyer_id,
dc.cat_name,
...
--conf "spark.app.name=ads_vova_buyer_page_category_top_behave" \
