...
sql="
ALTER TABLE ads.ads_vova_buyer_portrait_brand_price_range_likes DROP if exists partition(pt = '$(date -d "${cur_date:0:10} -180day" +%Y-%m-%d)');

INSERT overwrite TABLE ads.ads_vova_buyer_portrait_brand_price_range_likes partition(pt='${cur_date}')
SELECT
/*+ REPARTITION(40) */
  buyer_id,
  brand_id,
...
(
  ...
  WHERE
    pt > date_sub( '${cur_date}', 30 )
    AND pt <= '${cur_date}' and brand_id > 0
  GROUP BY
    buyer_id,
    brand_id,
    price_range,
...
--conf "spark.app.name=ads_buyer_portrait_brand_price_range_likes_short_term" \
...
echo "ads_buyer_portrait_brand_price_range_likes_short_term end_time:"  `date +"%Y-%m-%d %H:%M:%S" -d "8 hour"`


sql="
ALTER TABLE ads.ads_vova_buyer_portrait_brand_price_range_likes_medium_term DROP if exists partition(pt = '$(date -d "${cur_date:0:10} -30day" +%Y-%m-%d)');
insert overwrite table ads.ads_vova_buyer_portrait_brand_price_range_likes_medium_term partition(pt='${cur_date}')
SELECT
  /*+ REPARTITION(30) */
  buyer_id,
  brand_id,
...
(
  ...
  FROM
    dws.dws_vova_buyer_goods_behave
  WHERE
    pt >= date_sub('${cur_date}', 60) AND pt < date_sub('${cur_date}', 15) and brand_id > 0 and clk_cnt > 0
  GROUP BY
    buyer_id,
    brand_id,
    price_range,
...
ALTER TABLE ads.ads_vova_buyer_portrait_brand_price_range_likes_long_term DROP if exists partition(pt = '$(date -d "${cur_date:0:10} -30day" +%Y-%m-%d)');
insert overwrite table ads.ads_vova_buyer_portrait_brand_price_range_likes_long_term partition(pt='${cur_date}')
select
/*+ REPARTITION(1) */
...
left join
  tmp.tmp_vova_dictionary_price_range_type prt
on (dg.shop_price+dg.shipping_fee) >=prt.min_val and (dg.shop_price+dg.shipping_fee) <prt.max_val
where fp.confirm_time < date_sub('${cur_date}', 60) and dg.brand_id > 0
group by
  fp.buyer_id,
  dg.brand_id,
  prt.id
...
ALTER TABLE ads.ads_vova_buyer_portrait_brand_price_range_likes_exp DROP if exists partition(pt = '$(date -d "${cur_date:0:10} -30day" +%Y-%m-%d)');
insert overwrite table ads.ads_vova_buyer_portrait_brand_price_range_likes_exp partition(pt='${cur_date}')
select
/*+ REPARTITION(15) */
  buyer_id,
  brand_id,
  price_range,
  log(18,if(weights_short*10000>323,323,weights_short*10000)+1)/2 as weights_short,
  log(15,if(weights_medium*10000>224,224,weights_medium*10000)+1)/2 as weights_medium,
  log(2,if(weights_long*1000>255,255,weights_long*1000)+1)/8 as weights_long,
  log(2,if((0.6 * weights_short+ 0.3 * weights_medium + 0.1 * weights_long)*10000>127,127,(0.6 * weights_short+ 0.3 * weights_medium + 0.1 * weights_long)*10000)+1)/7 as likes_weight_synth -- 综合偏好
from
(
  select
    buyer_id,
  ...
  (
    ...
    (
      ...
      from ads.ads_vova_buyer_portrait_brand_price_range_likes
      where pt='${cur_date}' and buyer_id>0 and brand_id>0 and min_clk_day_gap is not null
      GROUP by buyer_id
    ) r
    left join
    (select *
  ...
  union all 
    ...
    (
      ...
      from ads.ads_vova_buyer_portrait_brand_price_range_likes_medium_term
  ...
  union all 
    ...
    (
      ...
      from ads.ads_vova_buyer_portrait_brand_price_range_likes_long_term
  ...
  group by buyer_id, brand_id, price_range
) result
;
"
...
--conf "spark.dynamicAllocation.minExecutors=5" \
--conf "spark.dynamicAllocation.initialExecutors=20" \
--conf "spark.app.name=ads_vova_buyer_portrait_brand_price_range_likes_exp" \
...
echo "ads_vova_buyer_portrait_brand_price_range_likes_exp end_time:" `date +"%Y-%m-%d %H:%M:%S" -d "8 hour"`
