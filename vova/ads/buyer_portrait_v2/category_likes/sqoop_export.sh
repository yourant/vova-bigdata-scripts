...
sql="
drop table if exists themis.ads_buyer_portrait_category_likes_pre;
drop table if exists themis.ads_buyer_portrait_category_likes_new;
create table themis.ads_buyer_portrait_category_likes_new(
...
UNIQUE KEY \`buyer_id_cat_id\` (\`buyer_id\`,\`cat_id\`) USING BTREE
)ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=DYNAMIC COMMENT='用户画像子品类偏好统计表(有近15日点击数据)';
"
...
--hcatalog-table ads_buyer_portrait_category_likes_with_click_15d \
--columns buyer_id,cat_id,expre_cnt_1w,expre_cnt_15d,expre_cnt_1m,clk_cnt_1w,clk_cnt_15d,clk_cnt_1m,clk_valid_cnt_1w,clk_valid_cnt_15d,clk_valid_cnt_1m,collect_cnt_1w,collect_cnt_15d,collect_cnt_1m,add_cat_cnt_1w,add_cat_cnt_15d,add_cat_cnt_1m,ord_cnt_1w,ord_cnt_15d,ord_cnt_1m \
...
rename table themis.ads_buyer_portrait_category_likes to themis.ads_buyer_portrait_category_likes_pre,themis.ads_buyer_portrait_category_likes_new to themis.ads_buyer_portrait_category_likes;
EOF
echo "-------rename结束--------"
