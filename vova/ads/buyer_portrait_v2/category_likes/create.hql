-- 用户画像子品类偏好统计表
drop table if exists ads.ads_buyer_portrait_category_likes;
create table if  not exists  ads.ads_buyer_portrait_category_likes (
...
) PARTITIONED BY (pt string)  COMMENT '用户画像子品类偏好统计表'
...
-- 用户画像子品类偏好统计表品牌偏好度表
drop table if exists ads.ads_buyer_portrait_category_likes_weight;
create table if  not exists  ads.ads_buyer_portrait_category_likes_weight (
   `buyer_id`                    bigint COMMENT 'd_买家id',
   `cat_id`                      bigint COMMENT 'd_品类id',
   `likes_weight`                decimal(13,4) COMMENt 'i_品牌偏好度'
)PARTITIONED BY (pt string) COMMENT '用户画像子品类偏好统计表品牌偏好度表' STORED AS PARQUETFILE;
