...
sql="
ALTER TABLE ads.ads_buyer_portrait_second_category_likes_weight DROP if exists partition(pt = '$(date -d "${pre_date:0:10} -180day" +%Y-%m-%d)');
INSERT overwrite TABLE ads.ads_buyer_portrait_second_category_likes_weight PARTITION ( pt = '${pre_date}' )
SELECT
  nvl(t1.buyer_id,lw.buyer_id),
  nvl(t1.second_cat_id,lw.second_cat_id),
...
GROUP BY
  ...
  FULL JOIN (SELECT* from ads.ads_buyer_portrait_second_category_likes_weight where pt = to_date ( date_sub( '${pre_date}', 1 ))) lw
  ON t1.buyer_id = lw.buyer_id
  AND t1.second_cat_id = lw.second_cat_id
...
--conf "spark.app.name=ads_buyer_portrait_second_category_likes_1" \
...
sql="
ALTER TABLE ads.ads_buyer_portrait_second_category_likes DROP if exists partition(pt = '$(date -d "${pre_date:0:10} -180day" +%Y-%m-%d)');
insert overwrite table ads.ads_buyer_portrait_second_category_likes partition(pt='$pre_date')
SELECT
  /*+ REPARTITION(30) */
  buyer_id,
  second_cat_id,
...
INSERT OVERWRITE TABLE ads.ads_buyer_portrait_second_category_likes_with_click_15d
SELECT
/*+ REPARTITION(10) */
buyer_id,
second_cat_id,
...
ord_cnt_1m
FROM
ads.ads_buyer_portrait_second_category_likes
...
--conf "spark.dynamicAllocation.initialExecutors=20" \
--conf "spark.dynamicAllocation.maxExecutors=100" \
--conf "spark.app.name=ads_buyer_portrait_second_category_likes_2" \
--conf "spark.default.parallelism = 380" \
--conf "spark.sql.shuffle.partitions=380" \
--conf "spark.sql.adaptive.enabled=true" \
--conf "spark.sql.adaptive.join.enabled=true" \
