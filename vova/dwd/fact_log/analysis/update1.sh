...
sql="
with dwd_vova_log_analysis_arc as(
select
event_fingerprint,
datasource,
event_name,
...
enter_ts,
leave_ts
from dwd.dwd_vova_log_analysis_arc where pt='$pt'
)
...
event_name
from dwd_vova_log_analysis_arc aa
...
left join dim.dim_vova_goods g on aa.element_id = g.virtual_goods_id
where aa.element_id is not null and aa.element_id !=''
union all
SELECT
/*+ REPARTITION(100) */
aa.event_fingerprint,
...
left join dim.dim_vova_buyers b on aa.buyer_id = b.buyer_id and aa.datasource = b.datasource
left join dim.dim_vova_devices d on aa.device_id = d.device_id and aa.datasource = d.datasource
where aa.element_id is null or aa.element_id=''
