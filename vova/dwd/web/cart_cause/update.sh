...
sql="
drop table if exists tmp.tmp_dwd_vova_web_fact_cart_cause_glk_cause;
create table tmp.tmp_dwd_vova_web_fact_cart_cause_glk_cause as
...
from (
         ...
         from (
                  ...
                  where log.pt = '$cur_date'
                    ...
                    and log.page_code in ('product')
              ) t1) t2
where t2.event_name = 'data';

drop table if exists tmp.dwd_vova_web_fact_cart_cause_expre_cause;
create table tmp.dwd_vova_web_fact_cart_cause_expre_cause as
...
from (
         ...
         from (
                  select datasource,
                         ...
                         null test_info,
                         null recall_pool
                  from tmp.tmp_dwd_vova_web_fact_cart_cause_glk_cause
...
insert overwrite table dwd.dwd_vova_web_fact_cart_cause PARTITION (pt = '${cur_date}')
...
select /*+ REPARTITION(2) */
       ...
       pre_test_info,
       pre_recall_pool
from tmp.dwd_vova_web_fact_cart_cause_expre_cause;
"
#如果使用spark-sql运行，则执行spark-sql -e
spark-sql --conf "spark.app.name=dwd_vova_web_fact_cart_cause" -e "$sql"
#如果脚本失败，则报错
if [ $? -ne 0 ];then
  exit 1
fi
