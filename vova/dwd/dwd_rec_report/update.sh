...
expres,
is_brand,
brand_status,count(1) cnt
from (
         select nvl(gc.datasource, 'NA')                           datasource,
                ...
                case
                    ...
                    when page_code = 'search_result' and
                         list_type in ('/search_result_price_desc', '/search_result_price_asc') then 'rec_search_price'
                    ...
                    when page_code = 'search_result' and list_type = '/search_result_also_like'
                        then 'rec_search_noresult_recommend'
                    when (page_code = 'robert_guide_session' and list_type = '/robert_guide_session_list')
                        or (page_code = 'recommend_product_list' and list_type = '/robert_guide_also_like')
                        then 'rec_robot'
                    else 'others' end                              rec_page_code,
                CASE
                    WHEN datediff(gc.pt, d.activate_time) <= 0 THEN 'new'
