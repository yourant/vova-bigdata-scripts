...
FROM (
         ...
         FROM (
                  ...
                  FROM (SELECT domain_userid,
                               datasource,
                               row_number() OVER (PARTITION BY datasource,domain_userid ORDER BY pt, min_create_time) AS rank,
                               first_value(first_page_url)
                                           OVER (PARTITION BY datasource,domain_userid ORDER BY pt, min_create_time)  AS first_page_url,
                               first_value(parse_url(first_referrer, 'HOST'))
                                           OVER (PARTITION BY datasource,domain_userid ORDER BY pt, min_create_time)  AS first_referrer
                        FROM dwd.dwd_vova_fact_web_start_up su WHERE su.datasource in ('vova', 'airyclub')
                       ) su
                  WHERE su.rank = 1
              ) temp
     ) final
...
FROM (SELECT domain_userid,
      ...
      FROM (SELECT domain_userid,
                   datasource,
                   region_code,
                   row_number() OVER (PARTITION BY datasource,domain_userid ORDER BY pt DESC, min_create_time DESC) AS rank,
                   first_value(min_create_time) OVER (PARTITION BY datasource,domain_userid ORDER BY pt, min_create_time)            AS activate_time
            FROM dwd.dwd_vova_fact_web_start_up su WHERE su.datasource in ('vova', 'airyclub')
           ) su
      WHERE su.rank = 1) t1
         LEFT JOIN (
    ...
    FROM (SELECT domain_userid,
          ...
          WHERE su.buyer_id > 0 AND su.datasource in ('vova', 'airyclub')) su
    WHERE su.rank = 1
) t2 ON t1.domain_userid = t2.domain_userid AND t1.datasource = t2.datasource
         LEFT JOIN dim.dim_vova_buyers db ON t2.buyer_id = db.buyer_id AND db.datasource = t2.datasource
         LEFT JOIN tmp.tmp_vova_dwd_web_first_url_data temp_url ON temp_url.domain_userid = t1.domain_userid AND temp_url.datasource = t1.datasource
         left join (
         select reg.domain_userid,datasource,
         min(reg.min_create_time) as register_success_time
         from tmp.tmp_vova_web_main_process_register reg
         group by reg.domain_userid, datasource
         ) register on register.domain_userid = t1.domain_userid AND register.datasource = t1.datasource
;
"

#如果使用spark-sql运行，则执行spark-sql --conf "spark.sql.parquet.writeLegacyFormat=true" -e
spark-sql --conf "spark.sql.parquet.writeLegacyFormat=true"  --conf "spark.dynamicAllocation.minExecutors=20" --conf "spark.dynamicAllocation.initialExecutors=40" --conf "spark.app.name=dwd_vova_fact_web_start_up" -e "$sql"
#如果脚本失败，则报错
if [ $? -ne 0 ];then
  exit 1
fi