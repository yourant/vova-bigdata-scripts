...
from
(
select pt,datasource,domain_userId,platform,buyer_id,event_name,geo_country,os_type,page_code,device_id,referrer,view_type,NULL element_name,NULL list_name from dwd.dwd_vova_log_page_view_arc where pt>=date_sub('${cur_date}', 1) and pt<='${cur_date}' and datasource NOT IN ('vova')
union all
select pt,datasource,domain_userId,platform,buyer_id,event_name,geo_country,os_type,page_code,device_id,referrer,NULL view_type,element_name,NULL list_name from dwd.dwd_vova_log_data_arc where pt>=date_sub('${cur_date}', 1) and pt<='${cur_date}' and datasource NOT IN ('vova')
union all
select pt,datasource,domain_userId,platform,buyer_id,event_name,geo_country,os_type,page_code,device_id,referrer,NULL view_type,element_name,NULL list_name from dwd.dwd_vova_log_click_arc where pt>=date_sub('${cur_date}', 1) and pt<='${cur_date}' and datasource NOT IN ('vova') and event_type='normal'
) t1
