...
DROP TABLE rpt.rpt_site_main_process;
CREATE TABLE IF NOT EXISTS rpt.rpt_site_main_process
(
    event_date           string COMMENT 'd_date',
    ...
    paid_order_cnt       bigint COMMENT 'i_支付订单数',
    first_order_cnt      bigint COMMENT 'i_新用户支付uv',
    order_cnt            bigint COMMENT 'i_下单数',
    is_new               string COMMENT 'd_用户激活时段类型',
    first_referrer_type  string COMMENT 'd_用户来源'
) COMMENT '' PARTITIONED BY (pt STRING)
    ROW FORMAT DELIMITED FIELDS TERMINATED BY '\001' STORED AS PARQUETFILE;
