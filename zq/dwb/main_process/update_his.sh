...
from
(
select pt,datasource,domain_userId,platform,buyer_id,event_name,geo_country,os_type,page_code,device_id,referrer,view_type,NULL element_name,NULL list_name from dwd.dwd_vova_log_page_view_arc where pt>=date_sub('${cur_date}', 30) and pt<='${cur_date}' and datasource NOT IN ('vova')
union all
select pt,datasource,domain_userId,platform,buyer_id,event_name,geo_country,os_type,page_code,device_id,referrer,NULL view_type,element_name,NULL list_name from dwd.dwd_vova_log_data_arc where pt>=date_sub('${cur_date}', 30) and pt<='${cur_date}' and datasource NOT IN ('vova')
union all
select pt,datasource,domain_userId,platform,buyer_id,event_name,geo_country,os_type,page_code,device_id,referrer,NULL view_type,element_name,NULL list_name from dwd.dwd_vova_log_click_arc where pt>=date_sub('${cur_date}', 30) and pt<='${cur_date}' and datasource NOT IN ('vova') and event_type='normal'
) t1
INNER JOIN dim.dim_zq_domain_userid fdu on fdu.domain_userid = t1.domain_userid AND fdu.datasource = t1.datasource
INNER JOIN dim.dim_zq_site zs on zs.datasource = t1.datasource AND zs.domain_group = 'FN'
) log_data
group by cube(log_data.datasource, log_data.region_code, log_data.platform, log_data.user_source, log_data.pt, nvl(log_data.is_new,'new'))
UNION ALL
select
...
FROM ods_zq_zsp.ods_zq_order_info oi
...
group by fin.event_date, fin.region_code, fin.platform, fin.user_source, fin.is_new, fin.datasource
...
FROM (
         ...
         FROM (
                  ...
                  GROUP BY CUBE (log.pt, log.platform, nvl(log.geo_country,'NALL'), nvl(fdu.original_channel, 'unknown'),
         ...
         GROUP BY final.pt, final.platform, final.region_code, final.user_source, final.is_new, final.datasource
         HAVING pt != 'all'
     ) fin
;
"
